...
static uint32_t imximage_csf_size;
		...
		uint32_t entry_point, uint32_t flash_offset)
{
	...
	fhdr_v2->self = hdr_base = entry_point - imximage_init_loadsize + flash_offset;
...
static void print_hdr_v2(struct imx_header *imx_hdr)
{
	...
	boot_data_t *boot_data = (boot_data_t*)&fhdr_v2->boot_data_ptr;
				...
				char *name, int lineno, int fld, int dcd_len)
{
	...
	case CMD_BOOT_FROM:
		...
		imximage_init_loadsize = get_table_entry_id(imximage_boot_loadsize,
					"imximage boot option", token);
	...
	case CMD_CSF:
		if (imximage_version != 2) {
			fprintf(stderr, "Error: %s[%d] - "
				"CSF only supported for IMAGE_VERSION 2"
				"(%s)\n", name, lineno, token);
			exit(EXIT_FAILURE);
		}
				...
				struct mkimage_params *params)
{
	...
	*header_size_ptr = ROUND(sbuf->st_size, 4096);
	...
	if (csf_ptr && imximage_csf_size) {
		*csf_ptr = (uint8_t*)params->ep - imximage_init_loadsize + *header_size_ptr;
	...
	struct image_type_params *tparams)
{
	...
	/* Parse dcd configuration file */
	dcd_len = parse_cfg_file(&imximage_header, params->imagename);
